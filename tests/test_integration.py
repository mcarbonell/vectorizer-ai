...
class TestVectorizerIntegration:
    ...
    async def test_vectorize_complete_flow_mock(self, test_image, output_path):
        ...
        # Mock generate
        mock_svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" fill="red"/></svg>'
        mock_generation = SVGGeneration(
            svg_code=mock_svg,
            metadata={"test": True}
        )
        
        with patch.object(vectorizer.vision_analyzer, 'analyze', new_callable=AsyncMock, return_value=mock_analysis):
            with patch.object(vectorizer.svg_generator, 'generate', new_callable=AsyncMock, return_value=mock_generation):
                with patch.object(vectorizer.svg_generator, 'modify', new_callable=AsyncMock, return_value=mock_generation):
                    with patch.object(vectorizer.image_comparator, 'render_svg'):
                        with patch.object(vectorizer.image_comparator, 'compare') as mock_compare:
                            ...
                            result = await vectorizer.vectorize_async(test_image, output_path)
    ...
    async def test_vectorize_with_iterations(self, test_image, output_path):
        ...
        mock_analysis = ImageAnalysis(
            shapes=[], colors=[], composition="", complexity="", style="", description=""
        )
                    ...
                    with patch.object(vectorizer.image_comparator, 'render_svg'):
                        with patch.object(vectorizer.image_comparator, 'compare') as mock_compare:
                            ...
                            mock_compare.side_effect = [
                                ComparisonResult(ssim=0.5, clip_similarity=0.5, quality_score=0.5, differences=[]),
                                ComparisonResult(ssim=0.7, clip_similarity=0.7, quality_score=0.7, differences=[]),
                                ComparisonResult(ssim=0.85, clip_similarity=0.85, quality_score=0.85, differences=[]),
                            ]
                ...
                with patch.object(vectorizer.svg_generator, 'modify', new_callable=AsyncMock, return_value=mock_generation):
                    with patch.object(vectorizer.image_comparator, 'render_svg'):
                        with patch.object(vectorizer.image_comparator, 'compare') as mock_compare:
                            ...
                            mock_compare.return_value = ComparisonResult(
                                ssim=0.9, clip_similarity=0.9, quality_score=0.9, differences=[]
                            )
                ...
                with patch.object(vectorizer.svg_generator, 'modify', new_callable=AsyncMock, return_value=mock_generation):
                    with patch.object(vectorizer.image_comparator, 'render_svg'):
                        with patch.object(vectorizer.image_comparator, 'compare') as mock_compare:
                            from vectorizer.models import ComparisonResult
                            mock_compare.return_value = ComparisonResult(
                                ssim=0.5, clip_similarity=0.5, quality_score=0.5, differences=[]
                            )
    ...
    async def test_vectorize_handles_render_error(self, test_image, output_path):
        ...
        with patch.object(vectorizer.vision_analyzer, 'analyze', new_callable=AsyncMock, return_value=mock_analysis):
            with patch.object(vectorizer.svg_generator, 'generate', new_callable=AsyncMock, return_value=mock_generation):
                with patch.object(vectorizer.image_comparator, 'render_svg') as mock_render:
                    # Simular error de renderizado
                    mock_render.side_effect = RuntimeError("No rendering method available")
...
class TestVectorizerSync:
    ...
    def test_vectorize_sync_calls_async(self, test_image, output_path):
        ...
        with patch.object(vectorizer, 'vectorize_async', new_callable=AsyncMock) as mock_async:
